...
namespace Ro.Npgsql.Data
{
    public interface IDbAsync
    {
        ...
        Task<T?> GetOneRow<T>(DbCommand cmd, Func<IDataReader, T> mapper) where T : class;
        Task<IEnumerable<T>> GetRows<T>(DbCommand cmd, Func<IDataReader, T> mapper);
        Task<T?> GetOneRowAsync<T>(DbCommand cmd, Func<IDataReader, Task<T>> mapper) where T : class;
